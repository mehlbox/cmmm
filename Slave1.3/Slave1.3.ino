...
void setup()
{
  ...
  pinMode(A2, INPUT); digitalWrite(A2,HIGH); // Jumper für i2c Adresse
  if (!digitalRead(A2)) MY_ADDRESS = 11; else MY_ADDRESS = 12; // i2c Adresse festlegen
...
void receiveEvent(int howMany) { // Daten erhalten
  ...
  switch (action) {
    ...
    case 13: // direct set of current position ####
      motor1.setCurrentPosition(recLong());
	    motor2.setCurrentPosition(recLong());
      if (motor1.currentPosition() != load(1)) save(motor1.currentPosition(), 1); // Aktuelle Position in EEPROM speichern
      if (motor2.currentPosition() != load(2)) save(motor2.currentPosition(), 2); // Aktuelle Position in EEPROM speichern 
      break;
...
void requestEvent() { // Daten wurden angefordert
    ...
    case 12: // Spannung abfragen ####
      voltage = analogRead(A3) * 27777.7777777777777; //  1000000/36
      sendLong(voltage);
      break;
