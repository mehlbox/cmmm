...
float spannung(int motor) { // Motorspannung
    int addr;
    long longVoltage;
    addr = caladdr(motor);
    busAction = 12;
...
void sendTargetValue() { // Zielwerte senden
    if (DEBUG) Serial.print("send target value: ");if (DEBUG) Serial.print(targetfrontline_steps);if (DEBUG) Serial.print(" ");if (DEBUG) Serial.println(targetbackline_steps);
    busAction = 3;
    byte data[9];
